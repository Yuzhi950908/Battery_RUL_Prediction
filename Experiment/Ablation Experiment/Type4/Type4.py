...
def train_4to1(epochs=300):
    ...
    torch.save(model.state_dict(), 'LSTM_4to1_exp4.pth')
    print("Final model saved to 'LSTM_4to1_exp4.pth'")
...
def train_4to4(epochs=500):
    ...
    for epoch in range(epochs):
        ...
        #    print(f"Early stopping triggered at epoch {epoch+1}")
        #    break
    torch.save(model.state_dict(), 'LSTM_4to4_exp4.pth')
    print("Final 4to4 model saved to 'LSTM_4to4_exp4.pth'")
...
def finetune_model(model_path, model_type, epochs=200, lr=1e-4):
    ...
    for epoch in range(epochs):
        ...
        #    break
    if model_type=="4to1":
        torch.save(model.state_dict(), 'finetuned_LSTM_4to1_B05_exp4.pth')
        print("Fine-tuned model saved to 'finetuned_LSTM_4to1_B05_exp4.pth'")
    #else:
...
def test():
    ...
    #model_4to1.load_state_dict(torch.load("finetuned_LSTM_4to1_B05_exp4.pth"))
    model_4to1.load_state_dict(torch.load("LSTM_4to1_exp4.pth"))
    model_4to4 = LSTM(sequence_length=30, n_features=4, hidden_size=16, num_layers=2, model_type="4to4")
    model_4to4.load_state_dict(torch.load("LSTM_4to4_exp4.pth"))
    model_4to4.eval()
    model_4to1.eval()
    with torch.no_grad():
...
for seed in Seeds:
    ...
    #finetune_model("LSTM_4to1_exp4.pth", "4to1")
...
df_results.to_csv("Type4_results_no_tf.csv", index=False)
######################################################################################################################################################################################################
