...
#Set some experiment paramerter
#########################################################################################################################################################################################################
#please note that, my each model has different train strategy, so its hard to build a high automatically run_experiment() function to gather all results,
#The total experiment is like that: I trained just windowsize 30  with different percentage(100%,70%,40%,10%) of fine-tunning datas
#Until all percentages are trained, this experiment finshed.

class experiment_Parameter(Enum):
    LSTM_type = ["4to1","4to4"]
    Seeds=[42, 123, 777, 888, 2025]
    Fine_Tuning_percentage=[1,0.7,0.4,0.1]

fine_tuning_percentage=0.1
seed=2025


random.seed(seed)
torch.manual_seed(seed)
past = future = sequence_length = window_size=30
dataset={}
...
#
    ...
    k_size=[x_train[0].shape[0],x_train[1].shape[0],x_train[2].shape[0]]
    x_train_all=torch.cat(x_train,dim=0) #torch.Size[291,30,1]
    y_train_all=torch.cat(label_train,dim=0) #torch.Size[291,30,1]

    return x_train_all,y_train_all,k_size



x_train_C,y_train_C,k_size=build_instance(df_C)
x_train_DVD,y_train_DVD,k_size=build_instance(df_DVD)
x_train_ICA,y_train_ICA,k_size=build_instance(df_ICA)
x_train_CCCT,y_train_CCCT,k_size=build_instance(df_CCCT)
...
def train_4to1(epochs=200):
    print("strat train LSTM_4to1")
    model = LSTM(sequence_length=sequence_length, n_features=4, hidden_size=16, num_layers=2,model_type="4to1")
    ...
    for epoch in range(epochs):
        ...
        battery_ids = list(range(len(k)))
        for battery_id in battery_ids:  # 0=B06, 1=B07, 2=B18
            h_t,c_t=None,None
            if battery_id == 0:
    ...
    torch.save(model.state_dict(), 'exp LSTM_4to1.pth')

#train_4to1()
print("Finish Model 4to1 training...")
print("=" * 50)



def train_4to4(epochs=300):
    print("start train LSTM_4to4")
    model = LSTM(sequence_length=sequence_length, n_features=4, hidden_size=16, num_layers=2, model_type="4to4")
    criterion = nn.MSELoss()
    optimizer = torch.optim.Adam(model.parameters(), lr=0.0001)
    ...
    for epoch in range(epochs):
        ...
        for battery_id in range(len(k)):  # 0=B06, 1=B07, 2=B18
            ...
            for idx in range(k[battery_id]):
                ...
                #if np.random.rand()<0.2 and idx!=0 and epoch>50:
        ...
        print(f"Epoch {epoch+1:3d} | Loss={avg_loss:.4f} | MAE={mae:.4f} | RMSE={rmse:.4f} | R²={r2:.4f}")
            ...
            #print(f"Early stopping triggered at epoch {epoch+1}")
            #break
    torch.save(model.state_dict(), 'exp LSTM_4to4.pth')
    

#train_4to4()
print("Finish Model 4to4 training...")
print("=" * 50)
...
finetune_dataset_4to1=MyDataset(x_fine_tuning,y_fine_tuning_4to1) #I create 109 training samples


def cut_dataset(data, percentage):
    cut_len = int(len(data) * percentage)
    data.x = data.x[:cut_len]
    data.y = data.y[:cut_len]
    return data


finetune_dataset_4to1=cut_dataset(finetune_dataset_4to1,fine_tuning_percentage)
...
def finetune_model(model_path, model_type, epochs=200, lr=1e-4):
    ...
    for epoch in range(epochs):
        ...
        print(f"Epoch {epoch+1:3d} | Fine-tune Loss = {avg_loss:.6f}")
            ...
            #print(f"Early stopping triggered at epoch {epoch+1}")
            #break
    torch.save(model.state_dict(), 'finetuned_exp_LSTM_4to1.pth')

#finetune_model('exp LSTM_4to1.pth',"4to1")
...
first_window_C = C_05[:window_size]
first_window_DVD = DVD_05[:window_size]#shape(30,)1d
first_window_ICA= ICA_05[:window_size]
first_window_CCCT= CCCT_05[:window_size]
first_window=np.stack([first_window_C,first_window_DVD,first_window_ICA,first_window_CCCT],axis=-1)#(30,4)
Tensor_test_B05 = torch.tensor(first_window.reshape(1, sequence_length, 4), dtype=torch.float32)
label_b05 = df_raw.iloc[:, 0].values
def test(sequence_length=sequence_length):
    model_4to1 = LSTM(sequence_length=sequence_length, n_features=4, hidden_size=16, num_layers=2,model_type="4to1")
    model_4to1.load_state_dict(torch.load('finetuned_exp_LSTM_4to1.pth'))
    model_4to4 = LSTM(sequence_length=sequence_length, n_features=4, hidden_size=16, num_layers=2, model_type="4to4")
    model_4to4.load_state_dict(torch.load('exp LSTM_4to4.pth'))
    model_4to4.eval()
    model_4to1.eval()
    with torch.no_grad():
        h_t,c_t=None,None
        ...
        for idx in range(500):
            ...
            if idx == 0:
                ...
                sliding_prediction.append(y_pre_4to1.detach().squeeze().tolist()[-1])
            ...
            if len(sliding_prediction) == expected_len:
                break
...
mae = mean_absolute_error(label_b05[window_size:], sliding_prediction_B05)
rmse = np.sqrt(mean_squared_error(label_b05[window_size:], sliding_prediction_B05))
r2 = r2_score(label_b05[window_size:], sliding_prediction_B05)
print(f"Metric for test dataset B05: MAE={mae:.4f} | RMSE={rmse:.4f} | R²={r2:.4f}")
with open("exp_result.csv", "a") as f:
    f.write(f"{fine_tuning_percentage},{mae:.4f},{rmse:.4f},{r2:.4f}\n")
