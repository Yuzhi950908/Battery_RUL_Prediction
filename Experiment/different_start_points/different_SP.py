...
#The total experiment is like that: i trained first windowsize 10(input 10, output 10) 5 times(with seeds 42,123,777,888,2025) without fine-tune and early-stopping, then recorded all metrics at the end.
#Until all windowsizes are trained, this experiment finshed.
...
past = future = sequence_length = window_size=50
seed=2025 
random.seed(seed)
...
def train_4to1(epochs=200):
    ...
    for epoch in range(epochs):
        ...
        torch.save(model.state_dict(), f'exp_4to1_seed{seed}_win{window_size}.pth')
    ...
    for epoch in range(epochs):
        ...
        for battery_id in range(len(k)):  # 0=B06, 1=B07, 2=B18
            ...
            for idx in range(k[battery_id]):
                ...
                #if np.random.rand()<0.2 and idx!=0 and epoch>50:#(if windowsizy=10,plese use this one)###
        ...
        r2 = r2_score(y_true_all, y_pred_all)
        avg_loss = np.mean(epoch_losses)
        torch.save(model.state_dict(), f'exp_4to4_seed{seed}_win{window_size}.pth')
        print(f"Epoch {epoch+1:3d} | Loss={avg_loss:.4f} | MAE={mae:.4f} | RMSE={rmse:.4f} | R²={r2:.4f}")
...
def test(sequence_length=sequence_length):
    ...
    model_4to1.load_state_dict(torch.load(f'exp_4to1_seed{seed}_win{window_size}.pth'))
    model_4to4 = LSTM(sequence_length=sequence_length, n_features=4, hidden_size=16, num_layers=2, model_type="4to4")
    model_4to4.load_state_dict(torch.load(f'exp_4to4_seed{seed}_win{window_size}.pth'))
    model_4to4.eval()
    model_4to1.eval()
    with torch.no_grad():
        h_t,c_t=None,None
...
with open("exp_result.csv", "a") as f:
    f.write(f"{window_size},{mae:.4f},{rmse:.4f},{r2:.4f}\n")
